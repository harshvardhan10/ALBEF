#!/usr/bin/env python3
"""Train original/lung/heart ALBEF with 2-layer anchored cross-attention fusion.
...
Three ViT-B/16 branches produce [B,257,768] token sequences. At each aligned
token position p, the original token is the anchor query and the lung/heart
tokens are a length-2 memory. Two Transformer-style cross-attention layers
update the original token:

    O_p  --CrossAttn([L_p,H_p])--> O'_p
    O'_p --CrossAttn([L_p,H_p])--> O''_p = F_p

Each layer uses 12-head cross-attention, residual + LayerNorm, then the same
768 -> 3072 -> 768 GELU feed-forward block and a second residual + LayerNorm.
No mean pooling is used. The resulting fused [B,257,768] sequence is optimized
with the standard ALBEF objectives:
    loss_total = loss_mlm + loss_ita + loss_itm

Initialization
--------------
For a fresh fusion run, the three visual branches are initialized from the
previously trained single-view A0 checkpoints. Shared text/projection/ITM
components come from the configured single-view checkpoint (original by
default). Old single-view queues are not loaded, and all momentum modules are
reset from the assembled online model.

Checkpoint policy
-----------------
The same dual VinDr validation policy is retained:
    checkpoint_best_cardiomegaly_auc.pth
    checkpoint_best_macro_auc_stable.pth

checkpoint_last.pth contains optimizer/scheduler state for crash recovery and
is optionally removed after a clean finish.
"""
...
from models.model_pretrain_multiview_cross_attention_fusion import ALBEF
...
EXPERIMENT_NAME = "A0_multiview_cross_attention_fusion_dual_auc"
...
def train_one_epoch(
    ...
    header = f"A0-multiview-cross-attn Epoch [{epoch}]"
...
def initialize_from_three_single_view_checkpoints(
    *,
    ...
    # The anchored cross-attention fusion module keeps its fresh initialization.
...
def save_last_checkpoint(
    *,
    ...
    payload = {
        ...
        "architecture": (
            "3x ViT-B/16 -> original query + [lung,heart] memory per position -> "
            "2-layer anchored cross-attention Transformer -> fused original "
            "token -> shared ALBEF ITC+ITM+MLM"
        ),
    }
    atomic_torch_save(
        payload,
