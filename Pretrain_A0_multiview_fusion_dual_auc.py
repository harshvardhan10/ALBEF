#!/usr/bin/env python3
"""Train a learned original/lung/heart token-fusion ALBEF model.
...
Three ViT-B/16 branches produce [B, 257, 768] token sequences. Corresponding
tokens are concatenated and passed through a learned FC fusion module
Linear(2304, 768) + LayerNorm. The one fused token sequence is then optimized
...
EXPERIMENT_NAME = "A0_multiview_fc_fusion_dual_auc"
...
def train_one_epoch(
    ...
    header = f"A0-multiview-fusion Epoch [{epoch}]"
...
def initialize_from_three_single_view_checkpoints(
    *,
    ...
    # The FC fusion layer remains at its equal-average initialization.
...
def save_last_checkpoint(
    *,
    ...
    payload = {
        ...
        "architecture": (
            "3x ViT-B/16 -> token concat -> Linear(2304,768) "
            "-> LayerNorm -> shared ALBEF ITC+ITM+MLM"
        ),
    }
    atomic_torch_save(
        payload,
