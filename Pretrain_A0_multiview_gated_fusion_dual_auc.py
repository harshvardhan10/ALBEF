#!/usr/bin/env python3
"""Train original/lung/heart ALBEF with image-level gated weighted fusion.

The dataset, initialization, ALBEF ITC/ITM/MLM objectives, momentum models,
queues, VinDr validation, dual checkpoint selection, DDP behavior, and training
hyperparameters are kept matched to the other multi-view fusion ablations.
"""
...
EXPERIMENT_NAME = "A0_multiview_gated_fusion_dual_auc"
...
def main(args, config):
    ...
    try:
        ...
        if completed_cleanly and utils.is_main_process() and last_path.exists():
            retain_last = bool(
                config.get("retain_last_checkpoint_after_success", False)
            )
            cardio_path = output_dir / "checkpoint_best_cardiomegaly_auc.pth"
            macro_path = output_dir / "checkpoint_best_macro_auc_stable.pth"

            if not retain_last:
                if cardio_path.exists() and macro_path.exists():
                    last_path.unlink()
                    print(
                        "[Checkpoint] Deleted checkpoint_last.pth only after "
                        "verifying that both validation-selected best "
                        "checkpoints exist.",
                        flush=True,
                    )
                else:
                    print(
                        "[WARNING] Retaining checkpoint_last.pth because one "
                        "or both validation-selected best checkpoints are "
                        "missing.",
                        flush=True,
                    )
                    print(
                        f"[WARNING] Cardiomegaly checkpoint exists: "
                        f"{cardio_path.exists()}; macro checkpoint exists: "
                        f"{macro_path.exists()}",
                        flush=True,
                    )
