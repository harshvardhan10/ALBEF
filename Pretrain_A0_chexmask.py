"""
Standard A0 ALBEF pretraining for a single CheXmask anatomical view.

The mask is applied to the original PIL image before the unchanged ALBEF
RandomResizedCrop / horizontal flip / RandAugment pipeline.
"""
...
from dataset.chexmask_view_dataset import CheXmaskPretrainDataset
...
def build_dataset(config):
    ...
    return CheXmaskPretrainDataset(
        ann_files=config["train_file"],
        transform=pretrain_transform,
        view_type=config["view_type"],
        chexmask_db=config.get("chexmask_db"),
        max_words=int(config.get("max_words", 30)),
        image_key=config.get("image_key", "image"),
        caption_key=config.get("caption_key", "caption"),
        mask_id_key=config.get("mask_id_key"),
        min_rca_mean=float(config.get("chexmask_min_rca_mean", 0.0)),
        failure_policy=config.get("chexmask_failure_policy", "error"),
        dilation_px=int(config.get("mask_dilation_px", 0)),
        feather_px=float(config.get("mask_feather_px", 0.0)),
    )
...
def train_one_epoch(
    ...
    header = f"A0-{config['view_type']} Epoch [{epoch}]"
    ...
    for step, (image, text) in enumerate(
        ...
        if step == 0 and utils.is_main_process():
            print(f"[DEBUG] view={config['view_type']}")
...
def save_checkpoint(
    ...
    obj = {
        ...
        "experiment": f"A0_chexmask_{config['view_type']}",
    }
