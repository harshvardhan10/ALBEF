#!/usr/bin/env python3
"""Extract ITC-margin ViT Grad-CAM maps from the transformer-fusion multi-view ALBEF model.
...
def load_fused_checkpoint(model: ALBEF, checkpoint_path: str | Path) -> None:
    ...
    print(f"[Checkpoint] Loaded transformer-fusion weights: {msg}")
...
def extract(args: argparse.Namespace) -> None:
    ...
    print(
        f"[ITC transformer-fusion] vit_layer_index={layer_index} "
        f"temperature={temperature:.6f}"
    )
    records = []
    try:
        for _, row in tqdm(df.iterrows(), total=len(df), desc="transformer-fusion ITC margin Grad-CAM"):
            ...
            out = {
                "__metadata__": {
                    ...
                    "method": "albef_multiview_transformer_itc_margin_self_attention_gradcam",
                    ...
                    "uses_itm": False,
                    "fusion_type": "two_layer_cross_view_transformer_then_mean_pool",
            ...
            for label_index, label in enumerate(args.target_labels):
                ...
                image_embeds = model._fuse_tokens(
                    z_original, z_lung, z_heart, momentum=False
                )
    ...
    index_path = output_dir / "transformer_fusion_itc_margin_gradcam_index.csv"
    ...
    with (output_dir / "transformer_fusion_itc_margin_gradcam_manifest.json").open("w", encoding="utf-8") as handle:
        json.dump(manifest, handle, indent=2)
    print(f"[Output] {index_path}")
    print("[Done] transformer-fusion ITC-margin heatmap extraction complete")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract transformer-fusion ALBEF ITC-margin ViT Grad-CAM heatmaps")
