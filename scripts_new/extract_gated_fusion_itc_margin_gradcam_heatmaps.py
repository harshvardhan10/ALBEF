#!/usr/bin/env python3
"""Extract ITC-margin ViT Grad-CAM maps from the gated-fusion multi-view ALBEF model.
...
from models.model_pretrain_multiview_gated_fusion import ALBEF
...
def load_fused_checkpoint(model: ALBEF, checkpoint_path: str | Path) -> None:
    ...
    print(f"[Checkpoint] Loaded gated-fusion weights: {msg}")
...
def extract(args: argparse.Namespace) -> None:
    ...
    print(
        f"[ITC gated-fusion] vit_layer_index={layer_index} "
        f"temperature={temperature:.6f}"
    )
    records = []
    try:
        for _, row in tqdm(df.iterrows(), total=len(df), desc="gated-fusion ITC margin Grad-CAM"):
            ...
            out = {
                "__metadata__": {
                    ...
                    "method": "albef_multiview_gated_itc_margin_self_attention_gradcam",
                    ...
                    "uses_itm": False,
                    "fusion_type": "image_level_cls_conditioned_softmax_gated_fusion",
            ...
            for label_index, label in enumerate(args.target_labels):
                ...
                image_embeds, view_weights = model.view_fusion(
                    z_original, z_lung, z_heart, return_weights=True
                )
                ...
                out[label] = {
                    ...
                    "fused": cam_fused,
                    "view_weights": {
                        "original": float(view_weights[0, 0].detach().cpu()),
                        "lung": float(view_weights[0, 1].detach().cpu()),
                        "heart": float(view_weights[0, 2].detach().cpu()),
                    },
                }
                ...
                record[f"weight_original::{label}"] = float(view_weights[0, 0].detach().cpu())
                record[f"weight_lung::{label}"] = float(view_weights[0, 1].detach().cpu())
                record[f"weight_heart::{label}"] = float(view_weights[0, 2].detach().cpu())
    ...
    index_path = output_dir / "gated_fusion_itc_margin_gradcam_index.csv"
    ...
    with (output_dir / "gated_fusion_itc_margin_gradcam_manifest.json").open("w", encoding="utf-8") as handle:
        json.dump(manifest, handle, indent=2)
    print(f"[Output] {index_path}")
    print("[Done] gated-fusion ITC-margin heatmap extraction complete")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract gated-fusion ALBEF ITC-margin ViT Grad-CAM heatmaps")
