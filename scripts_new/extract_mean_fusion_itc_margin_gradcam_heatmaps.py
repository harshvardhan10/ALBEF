#!/usr/bin/env python3
"""Extract ITC-margin ViT Grad-CAM maps from the mean-fusion multi-view ALBEF model.
...
def load_fused_checkpoint(model: ALBEF, checkpoint_path: str | Path) -> None:
    ...
    print(f"[Checkpoint] Loaded mean-fusion weights: {msg}")
...
def extract(args: argparse.Namespace) -> None:
    ...
    print(
        f"[ITC mean-fusion] vit_layer_index={layer_index} "
        f"temperature={temperature:.6f}"
    )
    records = []
    try:
        for _, row in tqdm(df.iterrows(), total=len(df), desc="mean-fusion ITC margin Grad-CAM"):
            ...
            out = {
                "__metadata__": {
                    ...
                    "method": "albef_multiview_mean_itc_margin_self_attention_gradcam",
                    ...
                    "uses_itm": False,
                    "fusion_type": "parameter_free_equal_mean_fusion",
    ...
    index_path = output_dir / "mean_fusion_itc_margin_gradcam_index.csv"
    ...
    with (output_dir / "mean_fusion_itc_margin_gradcam_manifest.json").open("w", encoding="utf-8") as handle:
        json.dump(manifest, handle, indent=2)
    print(f"[Output] {index_path}")
    print("[Done] mean-fusion ITC-margin heatmap extraction complete")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract mean-fusion ALBEF ITC-margin ViT Grad-CAM heatmaps")
