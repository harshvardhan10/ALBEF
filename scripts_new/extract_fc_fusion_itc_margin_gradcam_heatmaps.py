#!/usr/bin/env python3
"""Extract ITC-margin ViT Grad-CAM maps from the FC-fused multi-view ALBEF model.

This is the multi-view FC-fusion counterpart of your single-view script.

For each requested finding, it explains the same bare-prompt ITC classifier:
    positive = "<finding>"
    negative = "no <finding>"
    target = (sim(fused_image, positive) - sim(fused_image, negative)) / temperature

The forward path is:
    original image  -> visual_encoder_original
    lung-masked     -> visual_encoder_lung
    heart-masked    -> visual_encoder_heart
    token-wise FC fusion (Linear(2304 -> 768) + LayerNorm)
    fused CLS -> vision_proj -> normalized ITC image embedding

We backpropagate the single fused ITC margin through all three branches and
extract branch-specific Grad-CAM maps from each branch's ViT self-attention.

For each image / label, the output contains:
    - original branch CAMs
    - lung branch CAMs
    - heart branch CAMs
    - final fused CAM formed by summing raw positive evidence across branches
      before normalization

One `.pt` file is written per image.
"""
...
def interpolate_multiview_positional_embeddings(
...
def load_fused_checkpoint(
    ...
    print(f"[Checkpoint] Loaded fused model weights: {msg}")
...
def build_fused_cam(
    cam_original: Dict[str, torch.Tensor],
    cam_lung: Dict[str, torch.Tensor],
    cam_heart: Dict[str, torch.Tensor],
...
def load_three_view_images(
    images_root: Path,
    image_id: str,
    lung_mask_root: Path,
    heart_mask_root: Path,
) -> Tuple[Image.Image, Image.Image, Image.Image, Path, Path, Path]:
    ...
    if not (original.size == lung_mask.size == heart_mask.size):
        raise ValueError(
            f"Size mismatch for {image_id}: "
            f"image={original.size}, lung_mask={lung_mask.size}, heart_mask={heart_mask.size}"
        )
...
def resolve_layer_index(num_layers: int, vit_layer: int) -> int:
    ...
    if index < 0 or index >= num_layers:
        raise ValueError(
            f"Resolved vit layer index {index} out of range for {num_layers} layers"
        )
    return index
...
def extract(args: argparse.Namespace) -> None:
    ...
    attn_o = getattr(blocks_o[layer_index], "attn", None)
    attn_l = getattr(blocks_l[layer_index], "attn", None)
    attn_h = getattr(blocks_h[layer_index], "attn", None)
    if attn_o is None or attn_l is None or attn_h is None:
        raise AttributeError("One or more selected ViT blocks have no .attn module")

    cap_o = VisualAttentionCapture(attn_o)
    cap_l = VisualAttentionCapture(attn_l)
    cap_h = VisualAttentionCapture(attn_h)
    ...
    print(
        f"[ITC FC fusion] vit_layer_index={layer_index} "
    ...
    try:
        for _, row in tqdm(df.iterrows(), total=len(df), desc="FC-fusion ITC margin Grad-CAM"):
            ...
            out = {
                "__metadata__": {
                    ...
                    "method": "albef_multiview_fc_itc_margin_self_attention_gradcam",
                    "implementation_version": "1.0-fc-multiview-branch-plus-fused",
                    ...
                    "uses_itm": False,
                    "fusion_type": "fc_linear_2304_to_768_plus_layernorm",
                }
            }
            ...
            for label_index, label in enumerate(args.target_labels):
                ...
                image_embeds = model._fuse_tokens(
                    z_original,
                    z_lung,
                    z_heart,
                    momentum=False,
    ...
    index_path = output_dir / "fc_fusion_itc_margin_gradcam_index.csv"
    ...
    with (output_dir / "fc_fusion_itc_margin_gradcam_manifest.json").open("w") as handle:
        json.dump(manifest, handle, indent=2)

    print(f"[Output] {index_path}")
    print("[Done] FC-fusion ITC-margin heatmap extraction complete")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract FC-fused multi-view ALBEF ITC-margin ViT Grad-CAM heatmaps"
    )
    ...
    parser.add_argument(
        ...
        help='Example: --target_labels Cardiomegaly "Pleural Effusion"',
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-2,
        help="ViT block index to hook. Default -2 = second-to-last block.",
    )
    ...
    parser.add_argument(
        ...
        help="HF model name or path used to instantiate the ALBEF text encoder",
    )
    parser.add_argument(
        "--tokenizer_name_or_path",
        default=DEFAULT_BERT_CACHE,
        help="Local path or HF name for the BERT tokenizer",
    )
